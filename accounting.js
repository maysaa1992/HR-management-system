'use strict'
 ...
 let trTbele = document.getElementById("trtabele");
 console.log("hello")

 window.onload= function getDataLs (){
 ...
 if(arrToObj[i].Department=="Administration"){
    ...
    let totalEmp=numAdministration+numDevelopment+numFinance+numMarketing;
    let totalsalayr =salarA+salaryD+salaryF+salaryM;
    let totalavg=totalsalayr + totalEmp;
     renderTable(numAdministration,numMarketing,numFinance,numDevelopment,salarA,salaryM,salaryF,salaryD);
     footerTable(totalEmp,totalsalayr,totalavg);
}
 
function footerTable(TE,TS,TV){
 // document.write(`|| Administration||Marketing||Finance||Development||${TE}||${TS}||${TV}`);
 let thElTe =document.createElement('th')
 thElTe.textContent=TE;
 trTbele.appendChild(thElTe);

  let thElTs =document.createElement('th')
  thElTs.textContent=TS;
  trTbele.appendChild(thElTs);

  let thElTv =document.createElement('th')
  thElTv.textContent=TV;
  trTbele.appendChild(thElTv);
}
...
// a.addEventListener("click", getDataLs);
